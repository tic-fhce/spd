...
#MODULO POSTULANTES
    public function listaPostulante($id_carrera){
        ...
        if(isset($_SESSION['usuario']))
        {
            ...
            if($datos->id_permiso<3)
                $postulante=DB::table('view_mispostulaciones')->where('id_carrera',$id_carrera)->get();
            else 
                $postulante=DB::table('view_mispostulaciones')->where('id_carrera',$datos->id_carrera)->get();
    ...
    public function listaconvocatoria($id_carrera){
        ...
        if(isset($_SESSION['usuario']))
        {
            $datos=$_SESSION['usuario'];
            if($id_carrera=='1')
                $convocatoria=DB::table('view_convocatoriaadmin')->get();
            else
