...
<table>
  <tr>
    <td width="20%"><h3>Elaborado por :</h3></td>
...
@foreach($lista as $l)
  <label>Convocatoria # {{$l->numeroconvocatoria}}</label>
  <strong>Para : {{$l->nombre}}</strong><br>
  <strong>Materia :</strong><a href="#"> {{$l->materia}} {{$l->sigla}}</a>
  <br>
  <table border="1" width="100%" cellspacing=0 cellpadding=2 bordercolor="666633">
      ...
      <tr>
          ...
          <td>{{$value->ci}}</td>
          <td>{{$value->nombre}} {{$value->apellido}}<br><font size="9px">{{$value->grado}}</font><br><font size="9px">Fecha de Registro {{$value->fecha}}</font><br>
              <font size="9px">{{$value->detalleps}}</font></td>
